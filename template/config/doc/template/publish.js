...
function buildNav(members) {
	...
	if (members.classes.length) {
		var groups = {},
			getProperGroupname = function getProperGroupname(string) {
		...
		for (var groupName in groups) {
			...
			if (classNav !== '') {
				nav += '<h3>' + getProperGroupname(groupName) + '</h3><ul>';
				nav += classNav;
				nav += '</ul>';
			}
		} // for
...
exports.publish = function(taffyData, opts, tutorials) {
	...
	Object.keys(helper.longnameToUrl).forEach(function(longname) {
		...
		if (myClasses.length) {
			...
			upCTitle = titlePrefix.charAt(0).toUpperCase() + titlePrefix.slice(1);
			generate(upCTitle + ': ' + myClasses[0].name, myClasses, helper.longnameToUrl[longname]);
		}
